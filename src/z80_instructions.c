...
void INC_HLm(Z80_CPU *z) { mem_val_set(z, *z->HL, mem_val(z, *z->HL) + 1); }
...
void CP_HLm(Z80_CPU *z) { cp(z, mem_HL(z)); }
...
void EX_SPm_HL(Z80_CPU *z) { uint16_t temp = mem_val(z, z->SP); mem_val_set(z, z->SP, *z->HL); *z->HL = temp; }
...
// Bit instruction set implementation


// IX instruction set implementation

// 0xE0
void POP_IX(Z80_CPU *z) { z->IX = combine(mem_val(z, z->SP + 1), mem_val(z, z->SP)); z->SP += 2; }


// IY instruction set implementation
...
Z80_Instruction Z80_FetchInstruction(Z80_CPU *z) {
    ...
    switch (op0) {
        case PREF_EXTD:
            ...
            instruction = op1 >= 0xA0 ? ExtendedInstructions[op1 - 0x60] : ExtendedInstructions[op1 - 0x40];
            break;
        case PREF_BIT:
            // Bit instruction set
            instruction = BitInstructions[op1];
            break;
        case PREF_IX:
            if (op1 == PREF_BIT) {
                ...
                instruction = IXInstructions[op1];
            }
            break;
        case PREF_IY:
            // IY instruction set
            if (op1 == PREF_BIT) {
                ...
                instruction = IYInstructions[op1];
            }
            break;
        default:
            // Main instruction set
            instruction = MainInstructions[op0];
            break;
    }
